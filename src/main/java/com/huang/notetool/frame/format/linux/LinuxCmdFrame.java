 ...
 * @date 2019-06-19
 */
public class LinuxCmdFrame extends JRootParentFrame implements Runnable {
    private static final int INITIAL_TABLE_ROW_NUM = 30;
    /**
     * 日志
     */
    private Logger logger = Logger.getLogger(LinuxCmdFrame.class);
    /**
     * Service
     */
     ...
     * @param ignored 忽略
     */
    public LinuxCmdFrame(String ignored) {
        logger.info("初始化显示面板");
    ...
    private JPanel initialSearchPanel() {
        ...
        linuxField.addKeyListener(linuxCmdKeyListener(linuxField, linuxNameIndex, usageField,
                permissionField));
        usageField.addKeyListener(usageFieldKeyListener(usageField, usageIndex, linuxField,
                permissionField));
        permissionField.addKeyListener(permissionFieldKeyListener(permissionField,
                permissionIndex, linuxField, usageField));
        return searchPanel;
    }
     ...
     * @return KeyAdapter
     */
    private KeyListener permissionFieldKeyListener(JTextField permissionField,
                                                   int permissionIndex, JTextField linuxField,
     ...
     * @return KeyAdapter
     */
    private KeyListener usageFieldKeyListener(JTextField usageField, int usageIndex,
                                              JTextField linuxField, JTextField permissionField) {
     ...
     * @return KeyAdapter
     */
    private KeyAdapter linuxCmdKeyListener(JTextField linuxField, int linuxNameIndex,
                                           JTextField usageField, JTextField permissionField) {
        ...
        return new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                ...
                TableRowSorter<DefaultTableModel> sorter =
                        (TableRowSorter<DefaultTableModel>) myTable.getSorter();
                if (sorter == null) {
    ...
    private ActionListener addBtnAction() {
        ...
        return e -> {
            addLinuxCmdFrame addLinuxCmdFrame = new addLinuxCmdFrame(LinuxCmdFrame.this);
    ...
    private ActionListener exportAction() {
        return e -> {
            ...
            String filePath = ChooseWayFrame.chooseFile(null, null,
                    JFileChooser.FILES_AND_DIRECTORIES, false, LinuxCmdFrame.this);
    ...
    private ActionListener importAction() {
        return e -> {
            ...
            String filePath = ChooseWayFrame.chooseFile(super.getContentPane(), txtFileFilter,
                    JFileChooser.FILES_ONLY, true, LinuxCmdFrame.this);
            if (StringUtils.isNotEmpty(filePath)) {
                logger.info("导入 Linux命令 数据准备中... ");
                String returnMsg = linuxCmdService.importMethod(filePath);
                new ShowMsgSinglePanel(returnMsg, LinuxCmdFrame.this);
                refreshData();
            }
        };
    }
    ...
    private ActionListener clearBtnAction() {
        return e -> {
            //1:否  2:取消  0:确定
            int result = JOptionPane.showConfirmDialog(LinuxCmdFrame.this, "确定清空数据？");
            if (result == 0) {
                linuxCmdService.deleteAll();
                JOptionPane.showMessageDialog(this, "成功清空数据");
                refreshData();
            } else {
                logger.info("清空Linux的用户选择：" + result);
            }
        ...
        return new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (row != -1) {
                    ...
                    //1:否  2:取消  0:确定
                    int result = JOptionPane.showConfirmDialog(LinuxCmdFrame.this, "确定删除？");
                    if (result == 0) {
                        ...
                        JOptionPane.showMessageDialog(LinuxCmdFrame.this, "删除成功");
                        refreshData();
                        int rowInTable = myTable.getTable().getRowCount();
                        int dbCount = linuxCmdService.countSize();
                        if (rowInTable > dbCount) {
                            myTable.getTable().setValueAt(myRender.getDefaultPanel(), dbCount,
                                    btnColumn);
                            myTable.getTable().updateUI();
                        }
                    }
        ...
        return e -> {
            ...
            if (row != -1) {
                ...
                if (StringUtils.isEmpty(id)) {
                    ...
                    JOptionPane.showMessageDialog(LinuxCmdFrame.this, "查看Linux命令失败，请重新选中此行");
                    return;
                }
                //需要新增按钮事件
    ...
    //                    if (btnColumn != null) {
    //                        //说明存在按钮
    //                        JPanel panel = (JPanel) table.getCellEditor(row, btnColumn)
    //                        .getTableCellEditorComponent(table,null,true,row,btnColumn);
    //                        Point point=table.getCellEditor(row, btnColumn)
    //                        .getTableCellEditorComponent(table,null,true,row,btnColumn)
    //                        .getMousePosition();
    //                        if (null == panel){
